...
TEST_F(DecimalArithmeticTest, multiply) {
  ...
  testDecimalExpr<TypeKind::HUGEINT>(
      makeFlatVector(std::vector<int128_t>{61}, DECIMAL(38, 7)),
      "c0 * c1",
      {makeFlatVector(std::vector<int128_t>{201}, DECIMAL(20, 5)),
       makeFlatVector(std::vector<int128_t>{301}, DECIMAL(20, 5))});

  // out_precision == 38, large values, trimming of scale.
  testDecimalExpr<TypeKind::HUGEINT>(
      makeFlatVector(
          std::vector<int128_t>{HugeInt::parse("201" + std::string(31, '0'))},
          DECIMAL(38, 6)),
      "c0 * c1",
      {makeFlatVector(std::vector<int128_t>{201}, DECIMAL(20, 5)),
       makeFlatVector(
           std::vector<int128_t>{HugeInt::parse(std::string(35, '9'))},
           DECIMAL(35, 5))});
  ...
  testDecimalExpr<TypeKind::HUGEINT>(
      ...
      {makeFlatVector(
           ...
           DECIMAL(38, 20)),
       makeFlatVector(
           std::vector<int128_t>{HugeInt::parse(std::string(36, '9'))},
           DECIMAL(38, 20))});

  // out_precision == 38, very large values, trimming of scale (requires convert
  // to 256). should cause overflow.
  testDecimalExpr<TypeKind::HUGEINT>(
      makeNullableFlatVector<int128_t>({std::nullopt}, DECIMAL(38, 6)),
      "c0 * c1",
      {makeFlatVector(
           std::vector<int128_t>{HugeInt::parse(std::string(35, '9'))},
           DECIMAL(38, 4)),
  ...
  // Corner case.
  testDecimalExpr<TypeKind::HUGEINT>(
      makeFlatVector(std::vector<int128_t>{0}, DECIMAL(38, 37)),
      "c0 * c1",
      {makeFlatVector(std::vector<int128_t>{201}, DECIMAL(38, 38)),
       makeFlatVector(std::vector<int128_t>{301}, DECIMAL(38, 38))});
...
TEST_F(DecimalArithmeticTest, decimalDivTest) {
  ...
  testDecimalExpr<TypeKind::HUGEINT>(
      ...
      {makeLongDecimalVector({std::string(35, '9')}, 35, 6),
       makeFlatVector<int128_t>(std::vector<int128_t>{201}, DECIMAL(20, 3))});
