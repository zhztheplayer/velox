...
inline static A checkAndReduceScale(const A& in, int32_t delta) {
  if (delta <= 0) {
    ...
    DecimalUtil::divideWithRoundUp<A, A, A>(
        r, in, A(velox::DecimalUtil::kPowersOfTen[delta]), 0, 0, overflow);
...
class Multiply {
 public:
  ...
  inline static void apply(
    ...
    if (rPrecision < 38) {
      ...
      if (!overflow) {
        r = checkedMultiply<R>(
            result, R(velox::DecimalUtil::kPowersOfTen[aRescale + bRescale]));
      }
    } else if (a == 0 && b == 0) {
      ...
      if (deltaScale == 0) {
        ...
        if (UNLIKELY(totalLeadingZeros <= 128)) {
          ...
          auto result = DecimalUtil::convert<R>(reslarge, overflow);
          if (!overflow) {
            r = result;
          }
        } else {
          if (LIKELY(deltaScale <= 38)) {
            // The largest value that result can have here is (2^64 - 1) *
            // (2^63
            // - 1), which is greater than BasicDecimal128::kMaxValue.
            R result = DecimalUtil::multiply<R>(R(a), R(b), overflow);
            VELOX_DCHECK(!overflow);
            // Since deltaScale is greater than zero, result can now be at
            // most
            ...
            // We are multiplying decimal(38, 38) by decimal(38, 38). The
            // result should be a decimal(38, 37), so delta scale = 38 + 38 -
            // 37 = 39. Since we are not in the 256 bit intermediate value
            // case and we are scaling down by 39, then we are guaranteed that
            // the result is 0 (even if we try to round). The largest possible
            // intermediate result is 38 "9"s. If we scale down by 39, the
            // leftmost 9 is now two digits to the right of the rightmost
            // "visible" one. The reason why we have to handle this case
            // separately is because a scale multiplier with a deltaScale 39
            // does not fit into 128 bit. This result mismatches with Spark,
            // but spark result is nearly 0.
            r = R(0);
          }
        }
      }
 ...
 private:
  inline static int256_t reduceScaleBy(int256_t in, int32_t reduceBy) {
    ...
    VELOX_DCHECK_EQ(divisor % 2, 0); // multiple of 10.
