...
bool DoubleValues::testDoubleRange(
    double min,
    double max,
    bool hasNull) const {
  if (hasNull && nullAllowed_) {
...
std::unique_ptr<Filter> DoubleValues::mergeWith(
...
std::unique_ptr<Filter> createDoubleValues(
  ...
  if (values.size() == 1) {
    return std::make_unique<DoubleRange>(
        values.front(), false, false, values.front(), false, false, nullAllowed);
  }
  ...
  return std::make_unique<DoubleValues>(
