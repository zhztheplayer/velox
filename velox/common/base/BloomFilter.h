...
class BloomFilter : public BloomFilterHelper {
 public:
  ...
  /// Returns true if 'this' has been initialized with merge().
  bool isSet() const {
    return bits_.size() > 0;
  }
  ...
  static bool mayContain(const char* serializedBloom, uint64_t value) {
    BloomFilterView view(serializedBloom);
