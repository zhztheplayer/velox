...
TEST_P(HashJoinTest, dynamicFilters) {
  ...
  // Test with VARCHAR keys to verify string runtime filter functionality.
  {
    SCOPED_TRACE("VARCHAR keys");
    
    // Create probe vectors with VARCHAR keys - mix of matching and non-matching
    ...
    for (int32_t i = 0; i < numSplits; ++i) {
      auto rowVector = makeRowVector({
          makeFlatVector<StringView>(
              numRowsProbe,
              [&](auto row) {
                // Create keys: 70% non-matching [key_0 to key_34], 30% matching [key_35 to key_53]
                int keyNum = (row - i * 10) % 100;
                if (keyNum < 0) keyNum += 100;
                return StringView::makeInline(fmt::format("key_{}", keyNum));
              }),
          makeFlatVector<int64_t>(numRowsProbe, [](auto row) { return row; }),
      });
    ...
    // Create build vectors with VARCHAR keys in range [key_35, key_44] - 10 unique keys
          ...
          makeFlatVector<StringView>(
              ...
              [i](auto row) {
                return StringView::makeInline(
                    fmt::format("key_{}", 35 + ((row + i * (numRowsBuild / 5)) % 10)));
              }),
          makeFlatVector<int64_t>(numRowsBuild / 5, [](auto row) { return row; }),
      }));
    }

    std::vector<RowVectorPtr> stringKeyOnlyBuildVectors;
    for (int i = 0; i < 5; ++i) {
      stringKeyOnlyBuildVectors.push_back(makeRowVector(
          {makeFlatVector<StringView>(numRowsBuild / 5, [i](auto row) {
            return StringView::makeInline(
                fmt::format("key_{}", 35 + ((row + i * (numRowsBuild / 5)) % 10)));
          })}));
    }

    createDuckDbTable("t_str", stringProbeVectors);
    createDuckDbTable("u_str", stringBuildVectors);

    auto stringProbeType = ROW({"c0", "c1"}, {VARCHAR(), BIGINT()});

    auto stringBuildSide = PlanBuilder(planNodeIdGenerator, pool_.get())
                               .values(stringBuildVectors)
                               .project({"c0 AS u_c0", "c1 AS u_c1"})
                               .planNode();
    auto stringKeyOnlyBuildSide =
        PlanBuilder(planNodeIdGenerator, pool_.get())
            .values(stringKeyOnlyBuildVectors)
            .project({"c0 AS u_c0"})
            .planNode();

    // Inner join with VARCHAR keys
    {
      SCOPED_TRACE("VARCHAR Inner join");
      core::PlanNodeId probeScanId;
      core::PlanNodeId joinId;
      auto op = PlanBuilder(planNodeIdGenerator, pool_.get())
                    .tableScan(stringProbeType)
                    .capturePlanNodeId(probeScanId)
                    .hashJoin(
                        {"c0"},
                        {"u_c0"},
      ...
      HashJoinBuilder(*pool_, duckDbQueryRunner_, driverExecutor_.get())
          ...
          .config(
              ...
              "true")
          .referenceQuery(
              "SELECT t_str.c0, t_str.c1 + 1, t_str.c1 + u_str.c1 FROM t_str, u_str WHERE t_str.c0 = u_str.c0")
          .verifier([&](const std::shared_ptr<Task>& task, bool hasSpill) {
            SCOPED_TRACE(fmt::format("hasSpill:{}", hasSpill));
            auto planStats = toPlanStats(task->taskStats());
            if (hasSpill) {
    ...
    // Left semi join with VARCHAR keys
    {
      SCOPED_TRACE("VARCHAR Left semi join");
      core::PlanNodeId probeScanId;
      core::PlanNodeId joinId;
      auto op = PlanBuilder(planNodeIdGenerator, pool_.get())
                    .tableScan(stringProbeType)
      ...
      HashJoinBuilder(*pool_, duckDbQueryRunner_, driverExecutor_.get())
          ...
          .config(
              ...
              "true")
          .referenceQuery(
              "SELECT t_str.c0, t_str.c1 + 1 FROM t_str WHERE t_str.c0 IN (SELECT c0 FROM u_str)")
          .verifier([&](const std::shared_ptr<Task>& task, bool hasSpill) {
            SCOPED_TRACE(fmt::format("hasSpill:{}", hasSpill));
            auto planStats = toPlanStats(task->taskStats());
            if (hasSpill) {
    ...
    // Right semi join with VARCHAR keys
    {
      SCOPED_TRACE("VARCHAR Right semi join");
      core::PlanNodeId probeScanId;
      core::PlanNodeId joinId;
      auto op = PlanBuilder(planNodeIdGenerator, pool_.get())
                    .tableScan(stringProbeType)
      ...
      HashJoinBuilder(*pool_, duckDbQueryRunner_, driverExecutor_.get())
          ...
          .config(
              ...
              "true")
          .referenceQuery(
              "SELECT u_str.c0, u_str.c1 + 1 FROM u_str WHERE u_str.c0 IN (SELECT c0 FROM t_str)")
          .verifier([&](const std::shared_ptr<Task>& task, bool hasSpill) {
            SCOPED_TRACE(fmt::format("hasSpill:{}", hasSpill));
            auto planStats = toPlanStats(task->taskStats());
            if (hasSpill) {
    ...
    // Right join with VARCHAR keys
    {
      SCOPED_TRACE("VARCHAR Right join");
      core::PlanNodeId probeScanId;
      core::PlanNodeId joinId;
      auto op = PlanBuilder(planNodeIdGenerator, pool_.get())
                    .tableScan(stringProbeType)
      ...
      HashJoinBuilder(*pool_, duckDbQueryRunner_, driverExecutor_.get())
          ...
          .config(
              ...
              "true")
          .referenceQuery(
              "SELECT t_str.c0, t_str.c1 + 1, t_str.c1 + u_str.c1 FROM t_str RIGHT JOIN u_str ON t_str.c0 = u_str.c0")
          .verifier([&](const std::shared_ptr<Task>& task, bool hasSpill) {
            SCOPED_TRACE(fmt::format("hasSpill:{}", hasSpill));
            auto planStats = toPlanStats(task->taskStats());
            if (hasSpill) {
...
TEST_P(HashJoinTest, dynamicFiltersWithSkippedSplits) {
  ...
  // Test with VARCHAR keys to verify string runtime filter functionality with skipped splits.
  {
    SCOPED_TRACE("VARCHAR keys with skipped splits");
    
    // Create probe vectors with VARCHAR keys
    std::vector<RowVectorPtr> stringProbeVectors;
    std::vector<std::shared_ptr<TempFilePath>> stringTempFiles;
    for (int32_t i = 0; i < numSplits; ++i) {
      auto rowVector = makeRowVector({
          makeFlatVector<StringView>(
              numRowsProbe,
              [&](auto row) {
                return StringView::makeInline(fmt::format("key_{}", row - i * 10));
              }),
          makeFlatVector<int64_t>(numRowsProbe, [](auto row) { return row; }),
          makeFlatVector<int64_t>(
              numRowsProbe, [&](auto /*row*/) { return i % 2 == 0 ? 0 : i; }),
    ...
    // Create build vectors with VARCHAR keys in range [key_35, key_233]
    std::vector<RowVectorPtr> stringBuildVectors;
    for (int i = 0; i < 5; ++i) {
      stringBuildVectors.push_back(makeRowVector({
          makeFlatVector<StringView>(
              ...
              [i](auto row) {
                return StringView::makeInline(
                    fmt::format("key_{}", 35 + 2 * (row + i * numRowsBuild / 5)));
              }),
          makeFlatVector<int64_t>(numRowsBuild / 5, [](auto row) { return row; }),
      }));
    }
    ...
    auto stringProbeType = ROW({"c0", "c1", "c2"}, {VARCHAR(), BIGINT(), BIGINT()});
