...
TEST_F(VectorHasherTest, stringFilter) {
  ...
  // Analyze the data to populate uniqueValues_
  hasher->decode(*vector, rows);
  hasher->computeValueIds(rows, hashes);

  // Get the filter without null support.
  auto filter = hasher->getFilter(false);
  ASSERT_TRUE(filter != nullptr);
  ...
  // Test that the filter accepts values that were in the original data.
  ...
  // Test that the filter rejects values that were not in the original data.
  ...
  // Test with null support.
  auto filterWithNull = hasher->getFilter(true);
  ASSERT_TRUE(filterWithNull != nullptr);

  auto bytesValuesWithNull = dynamic_cast<common::BytesValues*>(filterWithNull.get());
...
TEST_F(VectorHasherTest, stringFilterWithLongStrings) {
  ...
  auto vector = makeFlatVector<StringView>(
      strings.size(), [&strings](vector_size_t row) {
...
TEST_F(VectorHasherTest, stringFilterDistinctOverflow) {
  ...
  for (int batch = 0; batch < numBatches; ++batch) {
    ...
    auto vector = makeFlatVector<StringView>(
        numRows, [&strings](vector_size_t row) {
          return StringView(strings[row]);
        });
